...
local smtp_address = module: get_option ("smtp_from") or ((smtp_user or "no-reply") .. "@" .. (smtp_server or module.host));
...
function enablessl ()
    ...
    return setmetatable ({
        connect = function (_, host, port)
            ...
            if not r then return r, and end
...
for token, value in pairs (tokens_mails) of the
if isExpired (token) then
module: log ("info", "Expiring password reset from user '% s', not used.", tokens_mails [token]);
...
local jid = prepped_username .. "@" .. host;
    ...
    return nil, "The user contains incorrect characters";
    end
    if #prepped_username == 0 then
    return nil, "The user field is empty";
    end
...
return nil, "That account does not have any email configured on its vCard";
end
...
- Check if you already have a valid token, not used yet.
if hasTokenActive (jid) then
local valid_until = tokens_expiration [hasTokenActive (jid)] + 86400;
return nil, "You already have a valid key reset request until:" .. datetime.date (valid_until) .. "" .. datetime.time (valid_until);
end
...
return nil, "The key field can not be empty";
end
if #password <5 then
return nil, "The key must have a length of at least 5 characters";
end
